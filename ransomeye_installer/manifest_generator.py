...
class ManifestGenerator:
    ...
    def generate_manifest(self) -> Dict:
        ...
        if self.resolver:
            ...
            # CRITICAL: Add orchestrator unit (Rust binary, not Python module)
            # Orchestrator is generated by SystemdWriter but not detected as a service module
            orchestrator_unit = {
                'name': 'ransomeye-orchestrator.service',
                'module': 'orchestrator',  # Not a Python module, but tracked for consistency
                'source_path': 'GENERATED_AT_INSTALL_TIME',  # Generated by SystemdWriter
                'install_path': '/etc/systemd/system/ransomeye-orchestrator.service',
                'sha256_hash': None,  # Populated by installer after unit installation
            }
            # Only add if not already present (avoid duplicates)
            if not any(u['name'] == 'ransomeye-orchestrator.service' for u in manifest['systemd_units']):
                manifest['systemd_units'].append(orchestrator_unit)
