...
impl TransportClient {
    pub fn new(config: Config, backpressure: Arc<BackpressureHandler>) -> Result<Self, TransportError> {
        // Check if URL is HTTP (no TLS needed)
        let url = url::Url::parse(&config.core_api_url)
            .map_err(|e| TransportError::TlsConfigFailed(format!("Invalid URL: {}", e)))?;
        
        let tls_config = if url.scheme() == "http" {
            // HTTP - create dummy TLS config (won't be used)
            Arc::new(ClientConfig::builder()
                .with_safe_defaults()
                .with_root_certificates(RootCertStore::empty())
                .with_no_client_auth())
        } else {
            // HTTPS - load certificates
    ...
    async fn try_send(&self, data: &[u8]) -> Result<(), TransportError> {
        ...
        if url.scheme() == "http" {
            // Use HTTP POST
            return self.try_send_http(&url, data).await;
        }
    ...
    async fn try_send_http(&self, url: &url::Url, data: &[u8]) -> Result<(), TransportError> {
        // Determine endpoint based on component type (for now, default to /ingest/linux)
        let endpoint = if url.path().ends_with("/ingest/linux") || url.path().ends_with("/ingest/dpi") {
            url.path().to_string()
        } else {
            // Default to /ingest/linux for Linux agent
            format!("{}/ingest/linux", url.path().trim_end_matches('/'))
        };
        
        let full_url = format!("{}://{}{}", url.scheme(), url.host_str().unwrap(), endpoint);
        
        // Parse JSON to determine if it's Linux or DPI
        let signed_event: serde_json::Value = serde_json::from_slice(data)
            .map_err(|e| TransportError::SendFailed(format!("Failed to parse event JSON: {}", e)))?;
        
        let component_identity = signed_event.get("component_identity")
            .and_then(|v| v.as_str())
            .unwrap_or("linux_agent");
        
        // Use /ingest/dpi if component_identity contains "dpi", otherwise /ingest/linux
        let endpoint_path = if component_identity.contains("dpi") {
            "/ingest/dpi"
        } else {
            "/ingest/linux"
        };
        ...
        let full_url = format!("{}://{}{}{}", 
            url.scheme(), 
            url.host_str().unwrap(),
            if url.port().is_some() { format!(":{}", url.port().unwrap()) } else { String::new() },
            endpoint_path
        );
        ...
        if !status.is_success() {
            ...
            return Err(TransportError::SendFailed(format!("HTTP {}: {}", status, body)));
        }
