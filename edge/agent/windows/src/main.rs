...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let process_handle = {
        ...
        tokio::spawn(async move {
            process_activity::ProcessActivityMonitor::monitor(run, tx, 5).await;
        })
    };
    ...
    let registry_handle = {
        let run = running.clone();
        let tx = event_tx.clone();
        let keys = config.monitor_registry_keys.clone();
        tokio::spawn(async move {
            registry_activity::RegistryActivityMonitor::monitor(run, keys, tx).await;
        })
    };
    ...
    let auth_handle = {
        ...
        let tx = event_tx.clone();
        tokio::spawn(async move {
            auth_activity::AuthActivityMonitor::monitor(run, tx, 10).await;
        })
    };
